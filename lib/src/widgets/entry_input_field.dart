...
/// Smart entry input field that adapts to medication form and provides:
/// - Mode toggle (tablets/strength for tablets, capsules/strength for capsules)
/// - Number input with steppers
/// - Quick action buttons (1/4, 1/2, 1, 2 for tablets)
/// - Live calculation display using EntryCalculator
///
/// Week 2 implementation covers: Tablets, Capsules, Pre-filled Injections, Single Entry Vials
...
class EntryInputField extends StatefulWidget {
  ...
  /// Callback when entry changes (returns EntryCalculationResult)
  final ValueChanged<EntryCalculationResult> onEntryChanged;
...
class _EntryInputFieldState extends State<EntryInputField> {
  ...
  EntryCalculationResult? _result;
    ...
    setState(() {
      ...
      switch (_mdvMode) {
        ...
        case MdvInputMode.volume:
          final ml = (result.entryVolumeMicroliter ?? 0) / 1000;
  ...
  EntryCalculationResult _computeResult(String text) {
    EntryCalculationResult result;
    ...
    if (_isCountMode) {
      ...
      switch (widget.medicationForm) {
        case MedicationForm.tablet:
          ...
          result = EntryCalculator.calculateFromTablets(
            tabletCount: count,
        ...
        case MedicationForm.capsule:
          ...
          if (count == null) {
            result = EntryCalculationResult.error(
              'Capsule count must be a whole number',
            );
            break;
          }
          result = EntryCalculator.calculateFromCapsules(
            capsuleCount: count,
        ...
        case MedicationForm.prefilledSyringe:
          ...
          result = EntryCalculator.calculateFromPrefilledInjections(
        ...
        case MedicationForm.singleDoseVial:
          ...
          result = EntryCalculator.calculateFromSingleDoseVials(
            vialCount: count,
        ...
        case MedicationForm.multiDoseVial:
          ...
          result = EntryCalculationResult.error('Use MDV mode toggle');
          break;
      }
      ...
      if (widget.totalVialStrengthMcg == null ||
        ...
        switch (_mdvMode) {
          case MdvInputMode.strength:
            // Input: desired entry strength → calc volume + units
            final strengthMcg = _convertDisplayUnitToMcg(value);
            result = EntryCalculator.calculateFromStrengthMDV(
          ...
          case MdvInputMode.volume:
            // Input: desired entry volume (in ml) → calc volume + units
            final volumeMicroliter = value * 1000; // Convert ml to µL
            result = EntryCalculator.calculateFromVolumeMDV(
          ...
          case MdvInputMode.units:
            ...
            result = EntryCalculator.calculateFromUnitsMDV(
              syringeUnits: value,
              totalVialStrengthMcg: widget.totalVialStrengthMcg!,
              totalVialVolumeMicroliter: widget.totalVialVolumeMicroliter!,
              syringeType: widget.syringeType!,
      ...
      switch (widget.medicationForm) {
        case MedicationForm.tablet:
          result = EntryCalculator.calculateFromStrength(
            strengthMcg: strengthMcg,
            strengthPerTabletMcg: widget.strengthPerUnitMcg,
          );
          break;
        ...
        case MedicationForm.capsule:
          result = EntryCalculator.calculateFromStrengthCapsules(
            strengthMcg: strengthMcg,
            strengthPerCapsuleMcg: widget.strengthPerUnitMcg,
          );
          break;
        ...
        case MedicationForm.multiDoseVial:
          ...
          result = EntryCalculationResult.error('Strength mode not supported');
          break;
      }
    }
  ...
  void _toggleMdvMode(MdvInputMode newMode) {
    ...
    widget.onEntryChanged(
      EntryCalculationResult.error('Mode changed - enter new entry'),
    );
  }
  ...
  void _onSyringeDragChanged(double newUnits) {
    ...
    if (result.success && !result.hasError) {
      switch (_mdvMode) {
        case MdvInputMode.strength:
          final strengthMcg = result.entryMassMcg ?? 0;
  ...
  Widget _buildTabletCapsuleHelperText() {
    ...
    return Text(
      'Select $countLabel to enter quantity. Select Strength to enter a entry amount; the app calculates the equivalent using the strength per $perUnitLabel.',
  ...
  Widget _buildMdvModeHelperText() {
    ...
    return Column(
      ...
      children: [
        Text(
          'Select a entry input method. The app calculates the other values automatically.',
          style: helperTextStyle(context),
        ),
    ...
    return LabelFieldRow(
      ...
      field: SmallDropdown36<String>(
        ...
        onChanged: (value) {
          ...
          final raw = double.tryParse(_controller.text.trim());
          // Convert the existing value to the new unit so the entry stays consistent.
          final mcg = raw == null ? null : _convertDisplayUnitToMcg(raw);
