...
import 'package:skedux/src/features/schedules/data/entry_calculation_service.dart';
...
import 'package:skedux/src/widgets/calendar/calendar_day_view.dart';
import 'package:skedux/src/widgets/calendar/calendar_header.dart';
import 'package:skedux/src/widgets/calendar/calendar_month_view.dart';
import 'package:skedux/src/widgets/calendar/calendar_week_view.dart';
import 'package:skedux/src/widgets/entry_card.dart';
import 'package:skedux/src/widgets/entry_action_sheet.dart';

import 'package:skedux/src/widgets/up_next_entry_card.dart';
import 'package:skedux/src/widgets/calendar/calendar_shared.dart';
import 'package:skedux/src/widgets/calendar/calendar_stage_panel.dart';
...
/// - Date navigation
/// - Entry filtering (by schedule or medication)
...
/// EntryCalendarWidget(variant: CalendarVariant.full)
///
...
/// )
/// ```
class EntryCalendarWidget extends StatefulWidget {
  const EntryCalendarWidget({
  ...
  @override
  State<EntryCalendarWidget> createState() => _EntryCalendarWidgetState();
}

class _EntryCalendarWidgetState extends State<EntryCalendarWidget> {
  late CalendarView _currentView;
  late DateTime _currentDate;
  DateTime? _selectedDate; // Track selected date for detail panel
  int? _selectedHour;
  List<CalculatedEntry> _entries = [];
  ...
  ValueListenable<Box<EntryLog>>? _entryLogsListenable;
  ...
  void initState() {
    ...
    // Auto-select today for month/week views to show today's entries
    ...
    // NOTE: _loadEntries() is deferred to didChangeDependencies so that
    // MaterialLocalizations.of(context) (used by _getDateRange) is safe to call.

    // Auto-refresh calendar when schedules or entry logs change.
    // This keeps the calendar up-to-date when a entry is marked taken/skipped/etc.
    _entryLogsListenable = Hive.box<EntryLog>('entry_logs').listenable();
    _entryLogsListenable!.addListener(_scheduleReload);
  ...
  @override
  void didUpdateWidget(EntryCalendarWidget oldWidget) {
  ...
  void dispose() {
    ...
    _entryLogsListenable?.removeListener(_scheduleReload);
  ...
  Future<void> _loadEntries() async {
    ...
    try {
      ...
      final entries = await EntryCalculationService.calculateEntries(
        startDate: startDate,
        endDate: endDate,
  ...
  void _onEntryTapInternal(CalculatedEntry entry) {
    if (widget.onEntryTap != null) {
      widget.onEntryTap!(entry);
      return;
    }
    _openEntryActionSheetFor(entry);
  }

  void _openEntryActionSheetFor(
    ...
    EntryActionSheet.show(
      ...
      onMarkLogged: (request) => _markEntryAsLogged(entry, request),
      onSnooze: (request) => _snoozeEntry(entry, request),
      onSkip: (request) => _skipEntry(entry, request),
      onDelete: (request) => _deleteEntryLog(entry),
    );
  }

  Future<void> _markEntryAsLogged(
    CalculatedEntry entry,
    EntryActionSheetSaveRequest request,
  ) async {
    final logId = EntryLogIds.occurrenceId(
      scheduleId: entry.scheduleId,
      scheduledTime: entry.scheduledTime,
    );
    final medicationId = entry.existingLog?.medicationId ?? 'unknown';
    final log = EntryLog(
      id: logId,
      scheduleId: entry.scheduleId,
    ...
    try {
      ...
      await _deductStock(
        entry,
        entryValueOverride: request.actualEntryValue,
        entryUnitOverride: request.actualEntryUnit,
        preferEntryValue: request.actualEntryValue != null,
      );

      // Cancel the notification for this entry
      await _cancelNotificationForEntry(entry);

      // Reload entries to reflect new status
      await _loadEntries();
  ...
  Future<void> _cancelNotificationForEntry(CalculatedEntry entry) async {
    try {
      ...
      debugPrint('[EntryCalendar] Failed to cancel notification: $e');
    }
  }

  Future<void> _snoozeEntry(
    CalculatedEntry entry,
    EntryActionSheetSaveRequest request,
  ) async {
    final logId = EntryLogIds.occurrenceId(
  ...
  Future<void> _deleteEntryLog(CalculatedEntry entry) async {
    if (entry.existingLog == null) return;
    ...
    try {
      ...
      // Restore stock if the entry was previously taken
      if (entry.existingLog!.action == EntryAction.logged) {
        final existing = entry.existingLog!;
        final oldEntryValue = existing.actualEntryValue ?? existing.entryValue;
        final oldEntryUnit = existing.actualEntryUnit ?? existing.entryUnit;
        await _restoreStock(
          entry,
          entryValueOverride: oldEntryValue,
          entryUnitOverride: oldEntryUnit,
          preferEntryValue: existing.actualEntryValue != null,
        );
      }
      ...
      if (mounted) {
        showAppSnackBar(context, 'Entry reset to pending');
      }
    } catch (e) {
      if (mounted) {
        showAppSnackBar(context, 'Error: $e');
  ...
  /// Deducts medication stock when a entry is taken
  Future<bool> _deductStock(
    CalculatedEntry entry, {
    double? entryValueOverride,
    String? entryUnitOverride,
    bool preferEntryValue = false,
  }) async {
    try {
      // Get the schedule to access medication info
      final scheduleBox = Hive.box<Schedule>('schedules');
      final schedule = scheduleBox.get(entry.scheduleId);
      ...
      final effectiveEntryValue = entryValueOverride ?? entry.entryValue;
      final effectiveEntryUnit = entryUnitOverride ?? entry.entryUnit;
      final delta = MedicationStockAdjustment.tryCalculateStockDelta(
        medication: med,
        schedule: schedule,
        entryValue: effectiveEntryValue,
        entryUnit: effectiveEntryUnit,
        preferEntryValue: preferEntryValue,
      );
      ...
      debugPrint('[EntryCalendar] Failed to deduct stock: $e');
      return false;
    }
  }

  /// Restores medication stock when a taken entry is reset
  Future<bool> _restoreStock(
    ...
    try {
      ...
      debugPrint('[EntryCalendar] Failed to restore stock: $e');
      return false;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    String? upNextEntryMetrics;

    if (nextEntry != null) {
      final scheduleBox = Hive.box<Schedule>('schedules');
      upNextSchedule = scheduleBox.get(nextEntry.scheduleId);
      ...
      if (med != null && schedule != null) {
        ...
        upNextEntryMetrics = schedule.displayMetrics(med);
      }
    }
        ...
        children: [
          ...
          if (widget.variant == CalendarVariant.full && widget.showUpNextCard)
            Padding(
              ...
              child: UpNextEntryCard(
                entry: nextEntry,
                onEntryTap: _onEntryTapInternal,
                onQuickAction: (status) {
                  ...
                  _openEntryActionSheetFor(d, initialStatus: status);
                },
                showMedicationName: true,
                medicationName:
                    upNextMedication?.name ?? nextEntry?.medicationName,
                strengthOrConcentrationLabel: upNextStrengthLabel,
                entryMetrics: upNextEntryMetrics,
            ...
            if (widget.variant == CalendarVariant.full && panelHeight != null)
              SizedBox(
                ...
                child: CalendarSelectedHourPanel(
                  ...
                  isFullVariant: true,
                  onEntryTap: _onEntryTapInternal,
                  onOpenEntryActionSheet: _openEntryActionSheetFor,
                ),
              )
            else
              Expanded(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (widget.embedInParentCard) {
          // When there is no entry-stage panel the body is already
  ...
  Widget _buildSelectedDayStageList({
    ...
    final dayEntries = _entries.where((entry) {
      return entry.scheduledTime.year == selectedDate.year &&
          entry.scheduledTime.month == selectedDate.month &&
          entry.scheduledTime.day == selectedDate.day;
    ...
    // Scrollable entry list — the handle/header are rendered OUTSIDE this widget
    // so they stay fixed (pinned) while the entries scroll underneath.
          ...
          itemBuilder: (context, index) {
            ...
            return Column(
              ...
              children: [
                ...
                _buildHourEntrySection(hour: hour, hourEntries: hourEntries),
              ],
            );
          },
        ),
  ...
  Widget _buildEntryCardFor(CalculatedEntry entry) {
    ...
    return EntryCard(
      ...
      onQuickAction: (status) {
        ...
        _openEntryActionSheetFor(entry, initialStatus: status);
      },
      onTap: () => _onEntryTapInternal(entry),
    );
  }
