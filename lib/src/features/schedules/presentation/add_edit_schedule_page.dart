...
class _ScheduleSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (medication == null || medicationName.isEmpty || doseValue <= 0 || doseUnit.isEmpty) {
      return Container(
        ...
        child: Row(
          children: [
            Icon(Icons.info_outline, color: cs.onSurfaceVariant.withValues(alpha: 0.5), size: 32),
    ...
    final timesStr = times.map((t) => t.format(context)).join(', ');
    String frequencyText;
    ...
    if (medication != null) {
      ...
      if ((medication!.form == MedicationForm.tablet && unitTxt == 'tablets') ||
          (medication!.form == MedicationForm.capsule && unitTxt == 'capsules')) {
    ...
    final endStr = noEnd || endDate == null ? 'No end' : '${endDate!.toLocal()}'.split(' ').first;

    // Calculate stock depletion
    var stockDepletionText = '';
    if (medication!.stockValue > 0) {
      ...
      if (dailyUsage > 0) {
        ...
        stockDepletionText = 'Estimated stock depletion by $depletionStr ($daysRemaining days)';
      }
    }
                    ...
                    Container(
                      ...
                      child: Text(
                        medication!.form == MedicationForm.tablet
                            ...
                            ? 'Capsule'
                            : medication!.form == MedicationForm.injectionPreFilledSyringe
                            ? 'PFS'
                            : medication!.form == MedicationForm.injectionSingleDoseVial
                            ? 'SDV'
                            : 'MDV',
