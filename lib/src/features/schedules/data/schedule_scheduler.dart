...
class ScheduleScheduler {
  ...
  static Future<void> scheduleFor(Schedule s) async {
    ...
    final days = useUtc ? (s.daysOfWeekUtc ?? const <int>[]) : s.daysOfWeek;
    final times = useUtc ? (s.timesOfDayUtc ?? [s.minutesOfDayUtc ?? s.minutesOfDay]) : (s.timesOfDay ?? [s.minutesOfDay]);
    for (final d in days) {
      for (final minutes in times) {
        int weekdayLocal;
        int minutesLocal;
        if (useUtc) {
          final slot = _utcToLocalSlot(d, minutes);
          weekdayLocal = slot.$1;
          minutesLocal = slot.$2;
        } else {
          weekdayLocal = d;
        ...
        final id = _slotId(s.id, weekday: d, minutes: minutes);
        await NotificationService.scheduleWeeklyAt(
          id: id,
          weekday: weekdayLocal,
          minutesOfDay: minutesLocal,
          title: title,
          body: body,
        );
      }
  ...
  static Future<void> cancelFor(String scheduleId, {Iterable<int>? days}) async {
    ...
    if (existing != null) {
      if (existing.hasCycle) {
        ...
        final daysList = useUtc ? (existing.daysOfWeekUtc ?? const <int>[]) : existing.daysOfWeek;
        final times = useUtc
            ? (existing.timesOfDayUtc ?? [existing.minutesOfDayUtc ?? existing.minutesOfDay])
            : (existing.timesOfDay ?? [existing.minutesOfDay]);
        for (final d in daysList) {
          for (final minutes in times) {
            final id = _slotId(existing.id, weekday: d, minutes: minutes);
            await NotificationService.cancel(id);
          }
        }
      }
