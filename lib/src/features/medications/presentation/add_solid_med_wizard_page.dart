...
enum SolidMedType { tablet, capsule }

extension SolidMedTypeConfig on SolidMedType {
  MedicationForm get form => switch (this) {
    SolidMedType.tablet => MedicationForm.tablet,
    SolidMedType.capsule => MedicationForm.capsule,
  };

  StockUnit get stockUnit => switch (this) {
    SolidMedType.tablet => StockUnit.tablets,
    SolidMedType.capsule => StockUnit.capsules,
  };

  /// Lowercase singular noun, e.g. "tablet" or "capsule".
  String get noun => switch (this) {
    SolidMedType.tablet => 'tablet',
    SolidMedType.capsule => 'capsule',
  };

  /// Capitalised singular noun, e.g. "Tablet" or "Capsule".
  String get capitalizedNoun => switch (this) {
    SolidMedType.tablet => 'Tablet',
    SolidMedType.capsule => 'Capsule',
  };

  IconData get icon => switch (this) {
    SolidMedType.tablet => Icons.medication,
    SolidMedType.capsule => Icons.medication_liquid,
  };
...
// ---------------------------------------------------------------------------
// AddSolidMedWizardPage — shared wizard for tablets and capsules
// ---------------------------------------------------------------------------

/// Wizard-style add/edit screen for solid-form medications (tablets & capsules).
class AddSolidMedWizardPage extends MedicationWizardBase {
  const AddSolidMedWizardPage({
    required this.solidMedType,
    super.key,
    super.initial,
    super.initialMedicationId,
  });

  final SolidMedType solidMedType;

  @override
  int get stepCount => 5;

  @override
  List<String> get stepLabels => const [
  ...
  @override
  ConsumerState<AddSolidMedWizardPage> createState() =>
      _AddSolidMedWizardPageState();
}

class _AddSolidMedWizardPageState
    extends MedicationWizardState<AddSolidMedWizardPage> {
  SolidMedType get _type => widget.solidMedType;
  ...
  void _loadInitialData() {
    ...
    if (m != null) {
      ...
      _lowStockThresholdCtrl.text =
          (m.lowStockThreshold?.toInt().clamp(1, 1000000) ?? 1).toString();
      _expiry = m.expiry;
  ...
  Widget buildSummaryContent() {
    ...
    final threshold = _lowStockEnabled
        ...
        : null;
    final headerTitle = name.isEmpty ? _type.capitalizedNoun : name;
        ...
        Row(
          ...
          children: [
            Container(
              ...
              child: Icon(_type.icon, color: fg),
            ),
                      ...
                      child: RichText(
                        text: TextSpan(
                          ...
                          children: [
                            ...
                            TextSpan(text: ' ${_type.noun}s in stock'),
                          ],
                        ),
                      ),
                    ),
  ...
  Widget _buildStrengthStep() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Enter the active ingredient amount per ${_type.noun}',
          style: mutedTextStyle(context),
        ),
        const SizedBox(height: 24),
        SectionFormCard(
          ...
          children: [
            ...
            buildHelperText(
              context,
              'Enter the active ingredient amount per ${_type.noun} (usually printed on packaging)',
              fullWidth: true,
            ),
          ],
        ),
  ...
  Widget _buildInventoryStep() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Track your ${_type.noun} inventory and set low stock alerts',
          style: mutedTextStyle(context),
        ),
        const SizedBox(height: 24),
        SectionFormCard(
          ...
          children: [
            ...
            buildHelperText(
              context,
              'Number of ${_type.noun}s you currently have',
              fullWidth: true,
            ),
            LabelFieldRow(
                ...
                field: StepperRow36(
                  ...
                  onInc: () {
                    ...
                    if (newVal == stock && (v + 1) > stock) {
                      showAppSnackBar(
                        context,
                        'Threshold cannot exceed current stock ($stock ${_type.noun}s)',
            ...
            buildHelperText(
              context,
              _lowStockEnabled
                  ? 'Alert when ${_type.noun}s drop to this level'
      ...
      children: [
        ...
        SectionFormCard(
          ...
          children: [
            ...
            buildHelperText(
              context,
              'When do the ${_type.noun}s expire?',
              fullWidth: true,
            ),
            LabelFieldRow(
              label: 'Batch No.',
            ...
            buildHelperText(
              context,
              'Where you keep the ${_type.noun}s',
              fullWidth: true,
            ),
            LabelFieldRow(
  ...
  Future<void> saveMedication() async {
    ...
    final med = Medication(
      ...
      lowStockThreshold: _lowStockEnabled
          ? (double.tryParse(_lowStockThresholdCtrl.text.trim()) ?? 1.0)
                .clamp(1.0, 1000000.0)
          : null,
      expiry: _expiry,
    ...
    try {
      ...
      debugPrint('AddSolidMedWizardPage(${_type.noun}): save failed: $e\n$stack');
      if (mounted) {
      ...
      children: [
        ...
        SectionFormCard(
          ...
          children: [
            ...
            _reviewRow('Type', _type.capitalizedNoun),
        ...
        SectionFormCard(
          ...
          titleStyle: reviewCardTitleStyle(context),
          children: [
            _reviewRow('Current Stock', '${fmt2(stock)} ${_type.noun}s'),
            _reviewRow(
              'Low Stock Alert',
              !_lowStockEnabled
                  ? 'Disabled'
                  : (threshold == null
                        ? 'Enabled'
                        : 'Enabled at ${fmt2(threshold)} ${_type.noun}s'),
            ),
          ],
        ),
        const SizedBox(height: 12),
