...
class _AddEditTabletDetailsStylePageState extends State<AddEditTabletDetailsStylePage> {
  ...
  Future<void> _save() async {
    ...
    if (_strengthValue == null || _strengthValue! <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter strength value')));
      return;
    }
    if (_stockValue == null || _stockValue! < 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter stock value')));
      return;
    }
  ...
  Future<void> _editText({required String title, required String initial, required ValueChanged<String> onSaved, TextInputType? keyboardType}) async {
    final ctrl = TextEditingController(text: initial);
    final ok = await showDialog<bool>(
          context: context,
          builder: (context) => AlertDialog(
            title: Text(title),
            content: TextField(controller: ctrl, keyboardType: keyboardType, autofocus: true, decoration: const InputDecoration()),
            actions: [
  ...
  Future<void> _editNumber({required String title, required double? initial, required ValueChanged<double?> onSaved}) async {
    final ctrl = TextEditingController(text: initial != null ? (initial == initial.roundToDouble() ? initial.toStringAsFixed(0) : initial.toString()) : '');
    final ok = await showDialog<bool>(
          context: context,
          builder: (context) => AlertDialog(
            title: Text(title),
  ...
  Future<void> _editEnum<T>({required String title, required T selected, required List<(T, String)> options, required ValueChanged<T> onSaved}) async {
  ...
  Future<void> _pickDate({required String title, required DateTime? initial, required ValueChanged<DateTime?> onSaved}) async {
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            SizedBox(width: 160, child: Text(label, style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w700, color: theme.colorScheme.onSurfaceVariant))),
            const SizedBox(width: 8),
            Expanded(child: Text(value.isEmpty ? '-' : value, style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurface))),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: GradientAppBar(
        title: widget.initial == null ? 'Add Medication (Details-style)' : 'Edit Medication (Details-style)',
        actions: [
        ...
        child: ListView(
          ...
          children: [
            _section(context, 'General', [
              _row('Name', _name, onTap: () => _editText(title: 'Name', initial: _name, onSaved: (v) => _name = v)),
              _row('Medication Type', 'Tablet'),
              _row('Manufacturer', _manufacturer, onTap: () => _editText(title: 'Manufacturer', initial: _manufacturer, onSaved: (v) => _manufacturer = v)),
              _row('Description', _description, onTap: () => _editText(title: 'Description', initial: _description, onSaved: (v) => _description = v)),
              _row('Notes', _notes, onTap: () => _editText(title: 'Notes', initial: _notes, onSaved: (v) => _notes = v)),
            ]),
            const SizedBox(height: 12),
            _section(context, 'Strength & Composition', [
              _row('Strength Value', fmt2(_strengthValue), onTap: () => _editNumber(title: 'Strength Value', initial: _strengthValue, onSaved: (v) => _strengthValue = v)),
              _row(
                'Strength Unit',
                switch (_strengthUnit) { Unit.mcg => 'mcg', Unit.mg => 'mg', Unit.g => 'g', Unit.units => 'units', Unit.mcgPerMl => 'mcg/mL', Unit.mgPerMl => 'mg/mL', Unit.gPerMl => 'g/mL', Unit.unitsPerMl => 'units/mL' },
                onTap: () => _editEnum<Unit>(
                  title: 'Strength Unit',
                  selected: _strengthUnit,
                  options: const [
                    (Unit.mcg, 'mcg'), (Unit.mg, 'mg'), (Unit.g, 'g'),
                  ],
            ...
            _section(context, 'Inventory', [
              _row('Stock Value', fmt2(_stockValue), onTap: () => _editNumber(title: 'Stock Value', initial: _stockValue, onSaved: (v) => _stockValue = v)),
              _row(
                'Stock Unit',
                switch (_stockUnit) {
              ...
              if (_lowStockEnabled)
                _row('Low stock threshold', fmt2(_lowStockThreshold), onTap: () => _editNumber(title: 'Low stock threshold', initial: _lowStockThreshold, onSaved: (v) => _lowStockThreshold = v)),
            ]),
            const SizedBox(height: 12),
            _section(context, 'Storage', [
              _row('Expiry', _expiry != null ? DateFormat('dd/MM/yy').format(_expiry!) : '-', onTap: () => _pickDate(title: 'Expiry', initial: _expiry, onSaved: (d) => _expiry = d)),
              _row('Batch', _batch, onTap: () => _editText(title: 'Batch', initial: _batch, onSaved: (v) => _batch = v)),
              _row('Storage location', _storageLocation, onTap: () => _editText(title: 'Storage location', initial: _storageLocation, onSaved: (v) => _storageLocation = v)),
              SwitchListTile(
                contentPadding: EdgeInsets.zero,
              ...
              _row('Storage instructions', _storageInstructions, onTap: () => _editText(title: 'Storage instructions', initial: _storageInstructions, onSaved: (v) => _storageInstructions = v)),
            ]),
          ],
        ),
      ),
