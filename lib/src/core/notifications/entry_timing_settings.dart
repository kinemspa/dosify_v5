...
class EntryTimingConfig {
  const EntryTimingConfig({
    required this.missedGracePercent,
    required this.overdueReminderPercent,
    required this.followUpReminderCount,
  });

  /// Percentage (0..100) of the time until the next scheduled entry
  /// after which a entry is considered "missed".
  ///
  /// Example: if the next entry is in 8 hours and this is 50, the entry becomes
  /// missed after 4 hours.
  final int missedGracePercent;

  /// Percentage (0..100) of the time before the entry is considered missed
  /// at which an "overdue" reminder should fire.
  ///
  /// Example: with missedGracePercent=50 and overdueReminderPercent=50,
  /// reminder fires at 25% of the interval to the next entry.
  ///
  /// Set to 0 to disable.
...
class EntryTimingSettings {
  const EntryTimingSettings._();

  static const String _prefsKeyMissedGracePercent =
      'entry_timing.missed_grace_percent_v1';
  static const String _prefsKeyOverdueReminderPercent =
      'entry_timing.overdue_reminder_percent_v1';
  static const String _prefsKeyFollowUpReminderCount =
      'entry_timing.follow_up_reminder_count_v1';
  ...
  static final ValueNotifier<EntryTimingConfig> value = ValueNotifier(
    const EntryTimingConfig(
  ...
  /// Returns the moment at which a entry should be considered "missed".
  static DateTime missedAt({
    ...
    // pct=0 => immediately missed; pct=100 => right at the next entry.
