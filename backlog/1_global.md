...
- [x] Enforce design-system compliance via an automated check for `Colors.*`, `EdgeInsets.*`, `BorderRadius.circular(...)` in `lib/src/features/**` (similar to the existing font-size checker).
...
- [x] Create a dedicated UI styling unification backlog item and track migration progress there (see `backlog/15_ui_styling_unification.md`).
- [x] Add an automated “no literal styling” gate for `lib/src/features/**`: block `Colors.*`, `Color(0x...)`, `EdgeInsets.*`, `BorderRadius.circular(...)`, `TextStyle(...)` unless explicitly allowed.
- [x] Standardize shared primitives (card surfaces, section headers, empty states, status chips/badges) in `lib/src/widgets/` so features don’t rebuild them.
