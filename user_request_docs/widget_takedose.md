...
## Status (Implemented)
- Heading + helper text [COMPLETED]
- Dose section shows a DoseCard when details available (fallback summary otherwise) [COMPLETED]
- Status selection chips + Save & Close flow [COMPLETED]
- Notes field [COMPLETED]
- Date & Time editing (for historical doses / existing logs) [COMPLETED]
- Ad-hoc amount editing (stepper) [COMPLETED]

## Status (Pending)
- Remove divider under helper text [PENDING]
- MDV/syringe visual graphic + multi-value dose presentation rules [PENDING]
- Snooze-until picker + prevent snoozing past next dose [PENDING]
- Center status chips + finalize per-status color behavior (taken/snoozed/skipped) [PENDING]
