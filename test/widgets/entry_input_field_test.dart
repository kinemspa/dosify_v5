...
void main() {
  group('EntryInputField - Tablets', () {
    ...
    testWidgets('quick button sets value and calculates', (tester) async {
      ...
      expect(result!.entryTabletQuarters, 8); // 2 tablets = 8 quarters
      expect(result!.entryMassMcg, 100000); // 2 × 50mg = 100mg
    });

    testWidgets('stepper buttons increment/decrement by 0.25', (tester) async {
      final results = <EntryCalculationResult>[];
        ...
        MaterialApp(
          home: Scaffold(
            body: EntryInputField(
              ...
              onEntryChanged: (r) => results.add(r),
            ),
          ),
        ),
      );
      ...
      expect(results.last.entryTabletQuarters, 5); // 1.25 tablets = 5 quarters
    });
    ...
    testWidgets('calculates from strength input', (tester) async {
      ...
      expect(result, isNotNull);
      expect(result!.success, true);
      expect(result!.entryTabletQuarters, 8); // 100mg ÷ 50mg = 2 tablets
    });
  });

  group('EntryInputField - Capsules', () {
    ...
    testWidgets('stepper increments by 1 for capsules', (tester) async {
      ...
      expect(lastResult.entryCapsules, 3); // 2 + 1 = 3
    });
    ...
    testWidgets('calculates from capsule count', (tester) async {
      ...
      expect(result!.entryMassMcg, 1500000); // 3 × 500mg = 1500mg
    });
  });

  group('EntryInputField - Pre-filled Injections', () {
    ...
    testWidgets('calculates from injection count', (tester) async {
      ...
      expect(result!.entryMassMcg, 600); // 2 × 300mcg
      expect(result!.entryVolumeMicroliter, 600); // 2 × 0.3ml
    });
  });

  group('EntryInputField - Single Entry Vials', () {
    testWidgets('calculates from vial count', (tester) async {
      ...
      expect(result!.entryMassMcg, 20000); // 2 × 10mg
      expect(result!.entryVolumeMicroliter, 4000); // 2 × 2ml
    });
  });

  group('EntryInputField - Error Handling', () {
  ...
  group('EntryInputField - Initial Values', () {
    testWidgets('initializes with tablet count', (tester) async {
      ...
      expect(result, isNotNull);
      expect(result!.entryTabletQuarters, 10); // 2.5 tablets
  ...
  group('EntryInputField - MDV (Multi-Dose Vial)', () {
    ...
    testWidgets('calculates from strength input', (tester) async {
      ...
      expect(result!.entryVolumeMicroliter, 250); // 500/2000 × 1000 = 0.25ml
    ...
    testWidgets('switches to Volume mode', (tester) async {
      ...
      expect(result!.entryMassMcg, 500); // 0.25/1.0 × 2000mcg = 500mcg
      expect(result!.syringeUnits, 25); // 0.25ml × 100 units/ml = 25 units
    });
    ...
    testWidgets('syringe tap updates all values', (tester) async {
      ...
      expect(result!.entryMassMcg, greaterThan(520));
      expect(result!.entryMassMcg, lessThan(560));
      expect(result!.entryVolumeMicroliter, greaterThan(260));
      expect(result!.entryVolumeMicroliter, lessThan(280));
    });
